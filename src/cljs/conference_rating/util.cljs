...
(defn location [l]
  [:div
   [:p {:class "conference-location conference-location-name" :data-e2e "text-conference-location-name" }
  ...
  (let [elem (.getElementById js/document elem-id)
    ...
    (doseq [i (range 1 (inc hop-count))]
      (let [hop-top-pos (* gap i)
            move-to (+ hop-top-pos doc-top negative-padding)
